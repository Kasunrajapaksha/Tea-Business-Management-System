<x-app-layout>
    ...
    @else
        @foreach ($notifications as $notification)
                ...
                <a href="{{ $notification->data['route'] }}" class="text-decoration-none">
                @endif

                    <div class="alert {{ $notification->read_at ? 'alert-secondary' : 'alert-' . $notification->data['color'] }} alert-dismissible d-flex align-items-center"
                        role="alert">
                        ...
                        <form action="{{ route('notifications.markAsRead') }}" method="post">
                            ...
                            @if ($notification->read_at)
                                <a href="#" class="btn btn-sm text-secondary ms-3 px-1 m-0 p-0"><i
                                        class="align-middle" data-feather="check"></i></a>
                            @else
                                <button type="submit"
                                    class="btn btn-sm text-{{ $notification->data['color'] }} ms-3 px-1 m-0 p-0"><i
