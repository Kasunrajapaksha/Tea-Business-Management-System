                        ...
                        @foreach ($invoices as $invoice)
                            <tr>
                                ...
                                <td class="d-none d-xl-table-cell">
                                    <a href="{{ route('marketing.document.show', $invoice) }}" class="btn btn-sm btn-primary">Review</a>
                                    <a href="{{ route('marketing.document.pdf', ['invoice' => $invoice->id, 'orders' => $invoice->order->pluck('id')->join(',')]) }}"
                                        class="btn btn-sm btn-info" target="_blank"><i class="align-middle" data-feather="eye"></i></a>
                                    <a href="{{ route('marketing.document.download', ['invoice' => $invoice->id, 'orders' => $invoice->order->pluck('id')->join(',')]) }}"
