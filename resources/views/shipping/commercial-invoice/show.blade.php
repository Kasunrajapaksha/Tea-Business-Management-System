    ...
    <div class="container-fluid p-0">
        ...
        <div class="row d-flex">
                <div class="card flex-fill">
                    <div class="card-body">
                        <form action="{{ route('shipping.invoice.store',[$customer,'orders' => $orders->pluck('id')->join(',')]) }}" method="POST">
                            @csrf
                            ...
                            <div class="row mb-3">
                                <div class="col-4">
                                    ...
                                    <select class="form-select" name="vessel_id" id="vessel_name_shedule_create"  {{ $orders->first()->shippingSchedule->vessel_id ? 'disabled' : ''}}>
                                        <option value="#">Select Vessel</option>
                                        @foreach ($vessels as $vessel)
                                            <option value="{{ $vessel->id }}" {{ $orders->first()->shippingSchedule->vessel_id == $vessel->id ? 'selected' : ''}}>{{ $vessel->vessel_name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-4">
                                    ...
                                    <select class="form-select" name="arrival_port" id="arrival_port_shedule_create" {{ $orders->first()->shippingSchedule->arrival_port ? 'disabled' : ''}}>
                                        <option value="#">Select port</option>
                                        @foreach ($ports as $port)
                                            <option value="{{ $port->port_name }}" {{ $orders->first()->shippingSchedule->arrival_port == $port->port_name ? 'selected' : ''}} >{{ $port->port_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row">
                                <div class="col-6">
                                    ...
                                    <select class="form-select" name="shipping_provider_id" {{ $orders->first()->shippingSchedule->shipping_provider_id ? 'disabled' : ''}} >
                                        <option value="#">Select Shipping Provider</option>
                                        @foreach ($providers as $provider)
                                        <option value="{{ $provider->id }}" {{ $orders->first()->shippingSchedule->shipping_provider_id == $provider->id ? 'selected' : ''}}>{{ $provider->provider_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                @if ($orders->first()->status == 17)
                                <a class="btn btn-primary mt-2 ms-2" data-bs-toggle="modal" data-bs-target="#updatePlan">Create Commercial Invoice</a>
                                @endif
                                @if ($orders->first()->status == 18)
                                <a href="{{ route('shipping.document.create', ['invoice' => $orders->first()->commercialInvoice->first()->id, 'orders' => $orders->pluck('id')->join(',')]) }}" class="btn btn-success mt-2 ms-2">Complete Shipment</a>
                                @endif
                                @if ($orders->first()->status == 19)
                                <a href="{{ route('shipping.document.edit', ['invoice' => $orders->first()->commercialInvoice->first()->id, 'orders' => $orders->pluck('id')->join(',')]) }}" class="btn btn-success mt-2 ms-2">Order Delivered</a>
                                @endif
                                ...
                                <div class="modal fade" id="updatePlan" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        ...
                                        <div class="modal-body">
                                            <h4>Are you sure you want to create the commercial invoice?</h4>
                                        </div>
